...
The same bar also has an optional `GEMINI_API_KEY` field (`lib/gemini-key-context.tsx`), stored
the same client-side way. Every demo reads it via `useGeminiKey()` to run its own judgment
against Gemini 3.5 Flash-Lite side by side with Jev's — same question, real latency, real cost
(from Gemini's published per-token pricing) — purely for comparison; Gemini's answer never drives
what a demo actually does. The shared plumbing lives in `lib/gemini.ts` (`askGemini`, one call
shape every demo's comparison action wraps around its own prompt and JSON schema) and
`components/CompareStrip.tsx` (the aggregate summary used by the batch demos).
...
- **`/parallel`** — the same 16 support tickets judged "is this about billing?" (`Noul`),
  ...
  fan-out has to happen on the server. A "Compare with Gemini" button runs the same batch through
  Gemini in parallel and shows timing, cost, and agreement rate against Jev.
- **`/citation`** — a hallucination detector: 8 claims checked against the policy documents they
  ...
  yes/no. Gemini comparison runs the same 8 claims through the equivalent 3-way classification.
- **`/tone`** — the passive-aggressiveness meter: 10 notes and messages scored on an escalation
  ...
  Gemini comparison here tends to show real disagreement (~60% agreement in testing) rather than
  the near-100% seen on the classification demos — scoring subjective intensity is a genuinely
  harder judgment to replicate than a category or a probability.
- **`/reflex`** — Reaction Match: guess "billing" or "not billing" on a ticket before Jev's own
  ...
  not a countdown — the opponent is Jev's actual latency, not a clock. With a Gemini key set, each
  round also fires the same judgment at Gemini in the background — never part of the race, just an
  extra "Gemini: Billing (Xms)" line once it answers.
- **`/jenga`** — Sentence Jenga: pull one word at a time out of a sentence; after each pull Jev
  ...
  *is* the tower's structural integrity — so you watch the meter go from solid (~0.95 while
  ...
  asks Gemini the same holds-meaning question in the background, purely for comparison.
- **`/bluff`** — the classic Indian card game Bluff (Cheat) against three AI opponents. Each
  ...
  Gemini in parallel — fire-and-forget, never consulted for the actual call.
...
Add a `GEMINI_API_KEY` there too if you want to try any demo's Gemini comparison.
